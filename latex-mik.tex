\documentclass[a4paper,draft,titlepage]{scrartcl}
...
\texttt{LaTeX\pfeil Environment on region} only works if a region is
selected ``linewise visual'' (with `V' in Vim's command mode). With a region
selected ``characterwise visual'' (`v') the script generates the error message: ``No
...
environment. As an example, \texttt{center} inserts
